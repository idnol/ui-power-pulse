import{b as c,q as ee,s as d,t as r,u as M,d as P,j as e,a as E,F as te,G as re,H as ie,D as L,J as Y,g as q,K as ne}from"./index-047d49db.js";import{M as oe,a as se,W as ae,S as le,T as de,C as ce,b as pe,V as he,c as xe,d as ue,B as me,e as ge,f as fe,g as be,N as we,I as ke,h as je,i as ye,j as $,k as ve,l as Se,m as Ce,n as Ae}from"./AddExerciseSuccess.styled-24b3da2e.js";import{a as Z}from"./ReusableStyles.styled-7ceb42ab.js";function Be(t){const[i,n]=c.useState([]);return c.useEffect(()=>{(async()=>{try{const s=await ee.get(`/exercises/filters?filter=${t}`);n(s.data)}catch(s){console.error("Error fetching categories:",s)}})()},[t]),i}const Ee=d.div`
position: relative;
`,Fe=d.div`
  max-width: 335px;
  margin-right: auto;
 

  @media (min-width: ${r.breakpoint.md}) {
    max-width: 100%;
    display: flex;
    align-items: baseline;
  }
`,Ue=d.div`
  display: flex;
  background-color: transparent;
  position: absolute;
  top: -35%;
  left: 0;
  @media (min-width: ${r.breakpoint.md}) {
    top: -30%;
  }
`,Le=d.button`
  display: flex;
  align-items: center;
  background-color: transparent;
  border: none;
  padding: 0;
  color: ${r.color.greyFifth};
`,Te=d.svg`
  width: 16px;
  height: 16px;
  margin-right: 6px;
`,Re=d.span`
  font-size: 14px;
  line-height: 1.285;
  &::first-letter {
    text-transform: uppercase;
  }
  @media (min-width: ${r.breakpoint.md}) {
    font-size: 16px;
    line-height: 1.33;
  }
`,Pe=d.h2`
  font-size: 24px;
  line-height: 1.16;
  max-width: 200px;
  text-transform: capitalize;
  margin-bottom: 20px;

  @media (min-width: ${r.breakpoint.md}) {
    display: flex;
    justify-content: baseline;
    max-width: 200px;
    line-height: 1.375;
    font-size: 32px;
    margin-bottom: 64px;
  }
`,Me=d.ul`
  display: flex;
  width: 244px;
  height: 26px;
  gap: 28px;
  margin-bottom: 40px;

  @media (min-width: ${r.breakpoint.md}) {
    gap: 32px;
    margin-left: auto;
    width: 279px;
    height: 32px;
    margin-bottom: 0px;
  }
`,N=d.li`
  line-height: 1.285;
  font-size: 14px;
  cursor: pointer;

  @media (min-width: ${r.breakpoint.md}) {
    line-height: 1.5;
    height: 24px;
    font-size: 16px;
  }
`,J=d.span`
  text-decoration: none;
  position: relative;
  color: ${r.color.greyFifth};
  cursor: pointer;
  &.active, &:hover {
    color: ${r.color.white};
    &:after {
      content: '';
      position: absolute;
      bottom: -9px;
      left: 50%;
      transform: translateX(-50%);
      width: 100%;
      height: 4px;
      background-color: ${r.color.orangeSecond};
      border-radius: 2px;
    }
  }
`;function $e({onClick:t,onRenderBackground:i}){const n=M();c.useEffect(()=>{i()});const[o,s]=c.useState("Body parts"),l=P(x=>x.filter.exercisesFilter);c.useEffect(()=>{t(o)},[t,o]);const a=x=>{t(x),s(x)},h=()=>{n(te())};return e.jsxs(Ee,{children:[e.jsx(Ue,{children:l!==""&&e.jsxs(Le,{onClick:h,children:[e.jsx(Te,{children:e.jsx("use",{href:`${E}#arrow-left-ex`,style:{fill:"rgba(239, 237, 232, 0.4)"}})}),e.jsx(Re,{children:"back"})]})}),e.jsxs(Fe,{children:[e.jsx(Pe,{children:l===""?"Exercises":l}),e.jsxs(Me,{id:"uniqueId",children:[e.jsx(N,{children:e.jsx(J,{className:o==="Body parts"?"active":"","data-target":"Body parts",onClick:()=>{a("Body parts"),h()},children:"Body parts"})}),e.jsx(N,{children:e.jsx(J,{className:o==="Muscles"?"active":"","data-target":"Muscles",onClick:()=>{a("Muscles"),h()},children:"Muscles"})}),e.jsx(N,{children:e.jsx(J,{className:o==="Equipment"?"active":"","data-target":"Equipment",onClick:()=>{a("Equipment"),h()},children:"Equipment"})})]})]})]})}d.div`
`;const Ne=d.ul`
    display: flex;
    flex-wrap: wrap;
    margin-bottom: 40px;

    @media (min-width: ${r.breakpoint.md}) {
        margin-right: -16px;
        margin-bottom: 32px;
    }

    @media (min-width: ${r.breakpoint.xl}) {
        margin-right: -16px; 
    }
`,Je=d.li`
    width: 100%;
    position: relative;
    margin-bottom: 20px;
    border-radius: 12px;
    overflow: hidden;
    
    @media (min-width: ${r.breakpoint.md}) {
        width: calc(33.3333% - 16px);
        margin-right: 16px;

        &:nth-child(3n) {
            margin-right: 0;
        }
    }


    @media (min-width: ${r.breakpoint.xl}) {
        width: calc(20% - 16px); 
        &:nth-child(3n) {
            margin-right: 16px;
        }
        &:nth-child(5n) {
            margin-right: 0;
        }
    }
`,Ie=d.div`
    position: relative;
    padding-bottom: 62%;
    &::after {
        content: "";
        display: block;
        position: absolute;
        width: 100%;
        height: 100%;
        top: 0;
        left: 0;
        background-color: rgba(4, 4, 4, 0.5); 
        border-radius: 12px;
        border: ${r.border.grey2};
        z-index: 1; 
     } 
     
    @media (min-width: ${r.breakpoint.md}) {
        padding-bottom: 92%;
    }
    @media (min-width: ${r.breakpoint.xl}) {
        padding-bottom: 87%;
    }
`,Ye=d.img`
    width: 100%;
    position: absolute;
    overflow: hidden;
    top: 0;
    height: 100%;
    object-fit: cover;
    background-image: url(${t=>t.src});
    background-size: cover;
    background-position: center;
`,qe=d.div`
    position: absolute;
    top: 50%;
    left: 50%;
    transform: translate(-50%, -50%);
    text-align: center;
    z-index: 2;
`,We=d.h3`
    font-size: 20px;
    line-height: 1.2;
    margin: 0;
    text-transform: lowercase;
    overflow: hidden;
    text-overflow: ellipsis;
    max-width: 176px;
    &:first-letter {
        text-transform: uppercase;
    }
`,Ke=d.h4`
    line-height: 1.5;
    font-size: 12px;
    margin: 2px 0 0;
    color: ${r.color.greyFifth};
`;function De({subcategory:t}){const i=M(),n=()=>{i(re({name:t.name,category:t.filter}))};return e.jsx(Je,{onClick:n,children:e.jsxs(Ie,{children:[e.jsx(Ye,{src:t.imgURL,alt:t.name}),e.jsxs(qe,{children:[e.jsx(We,{children:t.name}),e.jsx(Ke,{children:t.filter})]})]})})}const ze=d.div`
  display: flex;
  justify-content: center;
  align-items: center;
  position: absolute;
  left: 50%;
  bottom: -40px;
  transform: translateX(-50%);
  @media (min-width: ${r.breakpoint.md}) {
    bottom: -32px;
  }
`,Ze=d.button`
  width: 14px;
  height: 14px;
  border-radius: 50%;
  margin: 0 5px;
  background-color: ${t=>t.active?r.color.black:r.color.greySixth};
  border: ${t=>t.active?r.border.orange:"none"};
  position: relative;
  cursor: pointer;
  outline: none;
  vertical-align: middle;

  &:after {
    content: '';
    width: 8px;
    height: 8px;
    background-color: ${r.color.orange};
    border-radius: 50%;
    position: absolute;
    top: 50%;
    left: 50%;
    transform: translate(-50%, -50%);
    z-index: 1;
    display: ${t=>t.active?"block":"none"};
  }
`,Ge="/ui-power-pulse/assets/side-view-exercises-1x-desk-68380f68.jpg",Ve="/ui-power-pulse/assets/side-view-exercises-2x-desk-e1111ea7.jpg",Xe=t=>t.$isbackground?`url(${Ge})`:"none",Oe=t=>t.$isbackground?`url(${Ve})`:"none",He=d.div`
  @media (min-width: ${r.breakpoint.xl}) {
    border-bottom: ${r.border.grey2};
    background-color: ${r.color.black};
    width: 100%;
    height: 84px;
  }
`,Qe=d.div`
  position: relative;
`,_e=d.div`
  width: 100%; 
  @media (min-width: ${r.breakpoint.xl}) {
    padding-top: 72px;
  }
`,et=d.section`
  padding-top: 101px;
  @media (min-width: ${r.breakpoint.md}) {
    padding-top: 116px;
  }
  @media (min-width: ${r.breakpoint.xl}) {
    height: 100vh;
    width: 100%;
    padding-top: 0;
   background-image: ${Xe};
    background-repeat: no-repeat;
    background-position: right;
    background-size: contain;
    @media (min-device-pixel-ratio: 2),
      (min-resolution: 192dpi),
      (min-resolution: 2dppx) {
        background-image: ${Oe};
       }
  }
`,tt=d.div`
  position: relative;
   max-width: 335px;
   margin: 0 auto;
    @media (min-width: ${r.breakpoint.md}) {
    max-width: 686px;
  }
  @media (min-width: ${r.breakpoint.xl}) {
    max-width: 1248px;
  }
`;function rt({categories:t,selectedCategory:i}){const n=t.filter(p=>p.filter===i),[o,s]=c.useState(0),[l,a]=c.useState(10),h=768,x=Math.ceil(n.length/l),g=()=>{window.scrollTo({top:0,behavior:"smooth"})};c.useEffect(()=>{const p=()=>{window.innerWidth>=h&&window.innerWidth<=1439?a(9):a(10),s(0)};return window.addEventListener("resize",p),()=>{window.removeEventListener("resize",p)}},[]),c.useEffect(()=>{s(0),g()},[i]);const m=p=>{s(p),g()};return e.jsxs(tt,{children:[e.jsx(Ne,{children:n.slice(o*l,(o+1)*l).map((p,u)=>e.jsx(De,{subcategory:p},u))}),x>1&&e.jsx(ze,{children:e.jsx("div",{children:Array.from({length:x},(p,u)=>e.jsx(Ze,{active:u===o,onClick:()=>m(u),children:""},u))})})]})}const W="data:image/png;base64,",it="/ui-power-pulse/assets/like-2x-9c7b87f9.png",nt=({isOpen:t,onClose:i,calorise:n,time:o})=>{var a;const s=ie(),l=c.useRef(s);return e.jsxs(e.Fragment,{children:[e.jsx(oe,{onClick:i,children:e.jsx(se,{children:e.jsx("use",{href:`${E}#x-modal`})})}),t&&e.jsx(ae,{children:e.jsxs(le,{children:[e.jsxs("picture",{children:[e.jsx("source",{media:r.breakpoint.sm,srcSet:W}),e.jsx("source",{media:r.breakpoint.md,srcSet:it}),e.jsx("img",{src:W,alt:"Like",loading:"lazy"})]}),e.jsx(de,{children:"Well done"}),e.jsxs(ce,{children:[e.jsx(pe,{children:"Your time:"}),e.jsxs(he,{children:[o," minutes"]}),e.jsx(xe,{children:"Burned calories:"}),e.jsx(ue,{children:n})]}),e.jsx(me,{type:"button",onClick:i,children:"Next exercise"}),e.jsxs(ge,{to:((a=l.current.state)==null?void 0:a.from)??"/diary",children:["To the diary",e.jsx(fe,{children:e.jsx("use",{href:`${E}#arrow`})})]})]})})]})},ot=d.div`
display: flex;
flex-direction: column;
flex-wrap: wrap;
    padding: 48px 16px;
    width: 335px;
    height: 788px;
    margin-left: auto;
    margin-right: auto;
    @media (min-width: ${r.breakpoint.md}) {
        padding: 48px 30px;
        width: 694px;
        height: 550px;
    }
`,st=d.div`
    width: 270px;
    height: 226px;
    margin-left: auto;
    margin-right: auto;
    margin-bottom: 14px;
    border-radius: 12px;
    overflow: hidden;
    border: ${r.border.grey2};
    background: ${r.color.greySeven};
    img {
        width: 100%;
        height: 100%;
    }
`,at=d.div`
    display: flex;
    flex-direction: column;
    align-items: center;
    width: 270px;
    height: 214px;
    margin-left: auto;
    margin-right: auto;
    margin-bottom: 40px;
    @media (min-width: ${r.breakpoint.md}) {
        margin-bottom: 0;
    }
`,lt=d.ul`
    display: flex;
    flex-wrap: wrap;
    gap: 8px;
    width: 302px;
    height: 132px;
    margin-left: auto;
    margin-right: auto;
    margin-bottom: 24px;
    @media (min-width: ${r.breakpoint.md}) {
        margin-right: 0;
        width: 345px;
        height: 148px;
    }
`,dt=d.button`
    width: 151px;
    height: 42px;
    padding: 12px 32px;
    border-radius: 12px;
    border: none;
    margin-left: 0;
    color: ${r.color.white};
    font-size: 16px;
    font-weight: 500;
    line-height: 1.12;
    cursor: pointer;
    background-color: ${r.color.orange};
    &:hover{
        background-color: ${r.color.orangeSecond};
    };
    @media (min-width: ${r.breakpoint.md}) {
        margin-right: 0;
        height: 52px;
        padding: 14px 32px;
        line-height: 1.5;
        margin: auto 0 0 auto;
    }
`,ct=d.button`
    width: 32px;
    height: 32px;
    margin-bottom: 8px;
    margin-top: 14px;
    padding: 0;
    border: none;
    background-color: transparent;
    cursor: pointer;
`,pt=d(Z)`
    margin-bottom: 4px;
    font-size: 10px;
    line-height: 1.4;
`,ht=d.span`
    color: ${r.color.orange};
    font-size: 14px;
    line-height: 1.29;
`,xt=d.p`
    font-size: 14px;
    line-height: 1.29;
    color: ${r.color.grey};
`;var K=typeof window>"u"?c.useEffect:c.useLayoutEffect,ut=({isPlaying:t,duration:i,startAt:n=0,updateInterval:o=0,onComplete:s,onUpdate:l})=>{let[a,h]=c.useState(n),x=c.useRef(0),g=c.useRef(n),m=c.useRef(n*-1e3),p=c.useRef(null),u=c.useRef(null),w=c.useRef(null),S=j=>{let b=j/1e3;if(u.current===null){u.current=b,p.current=requestAnimationFrame(S);return}let k=b-u.current,y=x.current+k;u.current=b,x.current=y;let F=g.current+(o===0?y:(y/o|0)*o),v=g.current+y,T=typeof i=="number"&&v>=i;h(T?i:F),T||(p.current=requestAnimationFrame(S))},C=()=>{p.current&&cancelAnimationFrame(p.current),w.current&&clearTimeout(w.current),u.current=null},A=c.useCallback(j=>{C(),x.current=0;let b=typeof j=="number"?j:n;g.current=b,h(b),t&&(p.current=requestAnimationFrame(S))},[t,n]);return K(()=>{if(l==null||l(a),i&&a>=i){m.current+=i*1e3;let{shouldRepeat:j=!1,delay:b=0,newStartAt:k}=(s==null?void 0:s(m.current/1e3))||{};j&&(w.current=setTimeout(()=>A(k),b*1e3))}},[a,i]),K(()=>(t&&(p.current=requestAnimationFrame(S)),C),[t,i,o]),{elapsedTime:a,reset:A}},mt=(t,i,n)=>{let o=t/2,s=i/2,l=o-s,a=2*l,h=n==="clockwise"?"1,0":"0,1",x=2*Math.PI*l;return{path:`m ${o},${s} a ${l},${l} 0 ${h} 0,${a} a ${l},${l} 0 ${h} 0,-${a}`,pathLength:x}},D=(t,i)=>t===0||t===i?0:typeof i=="number"?t-i:0,gt=t=>({position:"relative",width:t,height:t}),ft={display:"flex",justifyContent:"center",alignItems:"center",position:"absolute",left:0,top:0,width:"100%",height:"100%"},G=(t,i,n,o,s)=>{if(o===0)return i;let l=(s?o-t:t)/o;return i+n*l},z=t=>{var i,n;return(n=(i=t.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i,(o,s,l,a)=>`#${s}${s}${l}${l}${a}${a}`).substring(1).match(/.{2}/g))==null?void 0:i.map(o=>parseInt(o,16)))!=null?n:[]},bt=(t,i)=>{var n;let{colors:o,colorsTime:s,isSmoothColorTransition:l=!0}=t;if(typeof o=="string")return o;let a=(n=s==null?void 0:s.findIndex((u,w)=>u>=i&&i>=s[w+1]))!=null?n:-1;if(!s||a===-1)return o[0];if(!l)return o[a];let h=s[a]-i,x=s[a]-s[a+1],g=z(o[a]),m=z(o[a+1]),p=!!t.isGrowing;return`rgb(${g.map((u,w)=>G(h,u,m[w]-u,x,p)|0).join(",")})`},wt=t=>{let{duration:i,initialRemainingTime:n,updateInterval:o,size:s=180,strokeWidth:l=12,trailStrokeWidth:a,isPlaying:h=!1,isGrowing:x=!1,rotation:g="clockwise",onComplete:m,onUpdate:p}=t,u=c.useRef(),w=Math.max(l,a??0),{path:S,pathLength:C}=mt(s,w,g),{elapsedTime:A}=ut({isPlaying:h,duration:i,startAt:D(i,n),updateInterval:o,onUpdate:typeof p=="function"?b=>{let k=Math.ceil(i-b);k!==u.current&&(u.current=k,p(k))}:void 0,onComplete:typeof m=="function"?b=>{var k;let{shouldRepeat:y,delay:F,newInitialRemainingTime:v}=(k=m(b))!=null?k:{};if(y)return{shouldRepeat:y,delay:F,newStartAt:D(i,v)}}:void 0}),j=i-A;return{elapsedTime:A,path:S,pathLength:C,remainingTime:Math.ceil(j),rotation:g,size:s,stroke:bt(t,j),strokeDashoffset:G(A,0,C,i,x),strokeWidth:l}},V=t=>{let{children:i,strokeLinecap:n,trailColor:o,trailStrokeWidth:s}=t,{path:l,pathLength:a,stroke:h,strokeDashoffset:x,remainingTime:g,elapsedTime:m,size:p,strokeWidth:u}=wt(t);return L.createElement("div",{style:gt(p)},L.createElement("svg",{viewBox:`0 0 ${p} ${p}`,width:p,height:p,xmlns:"http://www.w3.org/2000/svg"},L.createElement("path",{d:l,fill:"none",stroke:o??"#d9d9d9",strokeWidth:s??u}),L.createElement("path",{d:l,fill:"none",stroke:h,strokeLinecap:n??"round",strokeWidth:u,strokeDasharray:a,strokeDashoffset:x})),typeof i=="function"&&L.createElement("div",{style:ft},i({remainingTime:g,elapsedTime:m,color:h})))};V.displayName="CountdownCircleTimer";const kt=d.li`
    padding: 12px 18px;
    width: 147px;
    height: 62px;
    border-radius: 12px;
    border: ${r.border.grey2};
    background: ${r.color.greySeven};
    @media (min-width: ${r.breakpoint.md}) {
        width: 168px;
        height: 70px;
    }
`,jt=d.p`
    font-size: 12px;
    line-height: 1.33;
    color: ${r.color.grey};
    margin-bottom: 4px;
    @media (min-width: ${r.breakpoint.md}) {
        line-height: 1.5;
    }
`,yt=d(Z)`
    overflow: hidden;
    white-space: nowrap;
    text-overflow: ellipsis;
    font-weight: 700;
    color: ${r.color.white}
`,R=({tittle:t,name:i})=>e.jsxs(kt,{children:[e.jsx(jt,{children:t}),e.jsx(yt,{children:i})]}),vt=({item:t={name:"N/A",target:"N/A",bodyPart:"N/A",equipment:"N/A",gifUrl:"",burnedCalories:0,_id:"N/A"},onClose:i})=>{const[n,o]=c.useState(!1),[s,l]=c.useState(0),a=c.useRef(180),[h,x]=c.useState(0),[g,m]=c.useState(!1),[p,u]=c.useState(0),[w,S]=c.useState(0),{name:C,target:A,bodyPart:j,equipment:b,gifUrl:k,burnedCalories:y,_id:F}=t,v=180,T=M();c.useEffect(()=>{let f=null;return n&&(f=setInterval(()=>{if(n){const B=v-(a.current-1);l(Math.floor(B*y/v))}},1e3)),()=>{f&&clearInterval(f)}},[y,n]);const X=({remainingTime:f})=>{const U=Math.floor(f/60),B=f%60,I=B<10?`0${B}`:B;return`${U}:${I}`},O=()=>{!n&&a.current===0&&(x(f=>f+1),a.current=v,l(0)),o(!n)},H=()=>(u(f=>f+v-a.current),S(f=>f+s),x(f=>f+1),o(!1),l(0),{shouldRepeat:!1}),Q=async f=>{f.preventDefault();try{const U={exercise:F,time:v-a.current+p},B=await T(Y(U));Y.fulfilled.match(B)?m(!0):(m(!1),q.error("Oops, something went wrong"))}catch{m(!1),q.error("Oops, something went wrong")}},_=()=>{i(),m(!1)};return e.jsxs(e.Fragment,{children:[!g&&e.jsx("form",{onSubmit:Q,children:e.jsxs(ot,{children:[e.jsx(st,{children:e.jsx("img",{src:k,alt:C})}),e.jsxs(at,{children:[e.jsx(pt,{children:"Time"}),e.jsx(V,{isPlaying:n,size:124,strokeWidth:4,trailColor:`${r.color.greySixth}`,duration:180,colors:`${r.color.orange}`,onComplete:H,children:({remainingTime:f})=>(a.current=f,X({remainingTime:f}))},h),e.jsx(ct,{type:"button",onClick:()=>{O()},children:e.jsx("svg",{width:"32",height:"32",stroke:`${r.color.orange}`,viewBox:"0 0 32 32",children:n?e.jsx("use",{href:`${E}#pause-btn`}):e.jsx("use",{href:`${E}#play-btn`})})}),e.jsxs(xt,{children:["Burned calories:"," ",e.jsx(ht,{children:Math.floor(s+w)})]})]}),e.jsxs(lt,{children:[e.jsx(R,{tittle:"Name",name:C}),e.jsx(R,{tittle:"Target",name:A}),e.jsx(R,{tittle:"Body Part",name:j}),e.jsx(R,{tittle:"Equipment",name:b})]}),e.jsx(dt,{type:"submit",children:"Add to diary"})]})}),g&&e.jsx(nt,{calorise:Math.floor(s+w),time:Math.floor((v-a.current+p)/60),isOpen:g,onClose:()=>_()})]})},St=({item:t})=>{const[i,n]=c.useState(!1);return e.jsxs(e.Fragment,{children:[e.jsx(be,{children:"workout"}),e.jsxs(we,{children:[e.jsx(ke,{children:e.jsx("use",{href:`${E}#run-man-circle`})}),e.jsx(je,{children:t.name})]}),e.jsxs(ye,{children:[e.jsx($,{text:"Burned calories:",val:t.burnedCalories}),e.jsx($,{text:"Body part:",val:t.bodyPart}),e.jsx($,{text:"Target:",val:t.target})]}),e.jsxs(ve,{type:"button",onClick:()=>n(!0),children:[e.jsx(Se,{children:"Start"}),e.jsx(Ce,{children:e.jsx("use",{href:`${E}#arrow`})})]}),i&&e.jsx(Ae,{isOpen:i,onClose:()=>n(!1),children:e.jsx(vt,{item:t,onClose:()=>n(!1)})})]})},Ct=d.div`
  max-width: 335px;
  margin: 0 auto;
  @media (min-width: ${r.breakpoint.md}) {
    overflow-y: scroll;
    max-width: 704px;
    max-height: 686px;

    &::-webkit-scrollbar {
      width: 8px;
    }
    &::-webkit-scrollbar-track {
      background-color: ${r.color.greySixth};
      border-radius: 12px;
    }
    &::-webkit-scrollbar-thumb {
      background-color: ${r.color.orange};
      border-radius: 12px;
      height: 153px;
    }
  }
  @media (min-width: ${r.breakpoint.xl}) {
    max-width: 850px;
    max-height: 487px;
    margin: 0;
  }
`,At=d.ul`
  display: flex;
  flex-wrap: wrap;
  justify-content: flex-start;
  grid-row-gap: 20px;

  margin: 0 auto;

  @media (min-width: ${r.breakpoint.md}) {
    grid-row-gap: 32px;
    grid-column-gap: 16px;

    max-width: 704px;
  }

  @media (min-width: ${r.breakpoint.xl}) {
    max-width: 850px;
  }
`,Bt=d.li`
  position: relative;

  padding: 18px 16px 16px 16px;

  width: 100%;

  border-radius: 12px;
  border: ${r.border.grey2};
  background: rgba(239, 237, 232, 0.05);

  @media (min-width: ${r.breakpoint.md}) {
    max-width: 335px;
  }

  @media (min-width: ${r.breakpoint.xl}) {
    max-width: 405px;
  }
`,Et=({bodyPartFilter:t,category:i,onRenderBackground:n})=>{const o=M();let s="";switch(i){case"Body parts":s="bodyPart";break;case"Muscles":s="target";break;default:s=i.toLowerCase();break}const l={name:t.split(" ").join("-"),type:s};c.useEffect(()=>{o(ne(l))},[o]),c.useEffect(()=>{n()});const a=P(h=>h.exercises.items);return e.jsx(Ct,{children:e.jsx(At,{children:a.map((h,x)=>e.jsx(Bt,{children:e.jsx(St,{item:h})},x))})})};function Tt(){const[t,i]=c.useState(!1),[n,o]=c.useState(""),s=Be(n),l=P(m=>m.filter.exercisesFilter),a=P(m=>m.filter.category),h=m=>{o(m)},x=()=>{i(!0)},g=()=>{i(!1)};return e.jsxs(et,{$isbackground:t,children:[e.jsx(He,{}),e.jsx("div",{className:"container",children:e.jsx(Qe,{children:e.jsxs(_e,{children:[e.jsx($e,{onClick:h,onRenderBackground:g}),l===""?e.jsx(rt,{categories:s,selectedCategory:n}):e.jsx(Et,{bodyPartFilter:l,category:a,onRenderBackground:x})]})})})]})}export{Tt as default};
